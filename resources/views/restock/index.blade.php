        ...
        <div class="bg-black px-6 py-5 border-b border-slate-700 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 relative z-10">
            ...
            <div class="flex flex-col md:flex-row gap-3 w-full md:w-auto items-center">
                <form action="{{ route('restock.index') }}" method="GET" class="flex relative w-full md:w-auto">
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari barang kritis..." class="px-4 py-2.5 bg-slate-800 border border-slate-600 rounded-l-xl text-white text-sm focus:ring-2 focus:ring-red-500 w-full md:w-56 outline-none placeholder-slate-400">
                ...
                <form action="{{ route('restock.auto') }}" method="POST" class="w-full md:w-auto">
                    @csrf
                    {{-- <button type="submit" class="w-full bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-bold px-5 py-2.5 rounded-xl shadow-lg shadow-indigo-600/30 transition-all flex items-center justify-center" onclick="return confirm('Yakin mau lempar SEMUA barang kritis ini ke meja Manager otomatis? (Harga diset Rp 0 sementara)')">
                        <i class="fas fa-paper-plane mr-2"></i> Ajukan Semua (Auto Bulk)
                    </button> --}}
                </form>
            </div>
        </div>
            ...
            <table class="w-full text-left whitespace-nowrap">
                <thead class="bg-slate-50 dark:bg-slate-900 text-slate-500 dark:text-slate-400 text-xs uppercase font-bold border-b border-slate-200 dark:border-slate-700">
                    <tr>
                        ...
                        <th class="px-6 py-4 text-center">Sisa Stok (Real-time)</th>
                ...
                <tbody class="divide-y divide-slate-100 dark:divide-slate-700/50">
                    @forelse($products as $prod)
                        ...
                        <tr class="hover:bg-red-50 dark:hover:bg-red-900/10 transition-colors">
                            ...
                            <td class="px-6 py-4 text-center">
                                <button type="button" onclick="openRestockModal('{{ addslashes($nama) }}')" class="bg-slate-800 hover:bg-slate-900 text-white dark:bg-slate-700 dark:hover:bg-slate-600 text-xs font-bold px-5 py-2.5 rounded-xl shadow-md transition-all flex items-center justify-center mx-auto">
...
<div id="modalRestock" style="z-index: 9999;" class="fixed inset-0 items-center justify-center bg-slate-900 bg-opacity-80 hidden backdrop-blur-sm transition-opacity">
    <div class="bg-white dark:bg-slate-800 rounded-3xl shadow-2xl w-full max-w-2xl mx-4 overflow-hidden transform transition-all border border-slate-200 dark:border-slate-700">
        <div class="px-6 py-5 border-b border-gray-100 dark:border-slate-700 flex justify-between items-center bg-slate-50 dark:bg-slate-900/50">
            <div>
                <h3 class="font-extrabold text-lg text-slate-800 dark:text-white">Form Pengajuan Pembelian</h3>
                <p class="text-xs text-slate-500">Data akan dikirim ke antrean Manager (Purchase Plan)</p>
            </div>
            <button type="button" onclick="closeRestockModal()" class="text-gray-400 hover:text-red-500 transition-colors w-8 h-8 flex items-center justify-center bg-white dark:bg-slate-800 rounded-full shadow-sm"><i class="fas fa-times"></i></button>
        </div>
        <form action="{{ route('restock.process') }}" method="POST" class="p-6">
            @csrf
            <div class="mb-5">
                <label class="block text-xs font-bold text-slate-500 dark:text-slate-400 mb-2 uppercase tracking-wider">Barang yang Diajukan</label>
                <div class="relative">
                    <div class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                        <i class="fas fa-box text-slate-400"></i>
                    </div>
                    <input type="text" id="restock_product_name" name="product_name" class="w-full pl-10 pr-4 py-3 bg-slate-100 dark:bg-slate-900 border-0 rounded-xl font-bold text-lg cursor-not-allowed text-slate-700 dark:text-slate-300 ring-1 ring-inset ring-slate-200 dark:ring-slate-700 focus:ring-2 focus:ring-inset focus:ring-indigo-600" readonly>
                </div>
            </div>

            <div class="mb-6">
                <label class="block text-xs font-bold text-slate-500 dark:text-slate-400 mb-2 uppercase tracking-wider">Rekomendasi Supplier <span class="text-red-500">*</span></label>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-5 mb-8">
                <div>
                    <label class="block text-xs font-bold text-slate-500 dark:text-slate-400 mb-2 uppercase tracking-wider">Estimasi Harga <span class="text-red-500">*</span></label>
                    <div class="relative">
                        <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none text-slate-500 font-bold">Rp</div>
                        <input type="number" name="price" class="w-full pl-10 pr-4 py-3 bg-white dark:bg-slate-900 border-0 rounded-xl text-slate-800 dark:text-white font-bold ring-1 ring-inset ring-slate-300 dark:ring-slate-600 focus:ring-2 focus:ring-inset focus:ring-indigo-600" placeholder="0" required>
                    </div>
                </div>
                <div>
                    <label class="block text-xs font-bold text-slate-500 dark:text-slate-400 mb-2 uppercase tracking-wider">Jumlah Beli <span class="text-red-500">*</span></label>
                    <input type="number" name="qty" class="w-full px-4 py-3 bg-white dark:bg-slate-900 border-0 rounded-xl text-slate-800 dark:text-white font-bold ring-1 ring-inset ring-slate-300 dark:ring-slate-600 focus:ring-2 focus:ring-inset focus:ring-indigo-600 text-center" value="20" required>
                </div>
                <div>
                    <label class="block text-xs font-bold text-slate-500 dark:text-slate-400 mb-2 uppercase tracking-wider">Satuan <span class="text-red-500">*</span></label>
                    <select name="unit" class="w-full px-4 py-3 bg-white dark:bg-slate-900 border-0 rounded-xl text-slate-800 dark:text-white font-bold ring-1 ring-inset ring-slate-300 dark:ring-slate-600 focus:ring-2 focus:ring-inset focus:ring-indigo-600" required>
            ...
            <div class="flex justify-end gap-3 pt-5 border-t border-slate-100 dark:border-slate-700">
                <button type="button" onclick="closeRestockModal()" class="px-6 py-3 bg-white dark:bg-slate-700 border border-slate-300 dark:border-slate-600 rounded-xl font-bold text-slate-700 dark:text-white hover:bg-slate-50 transition-colors">Batal</button>
                <button type="submit" class="px-8 py-3 bg-slate-800 hover:bg-black text-white rounded-xl font-bold shadow-xl shadow-slate-800/30 transition-all flex items-center">
...
<script>
    document.addEventListener('DOMContentLoaded', function() { 
        $('.searchable-select').select2({ placeholder: "-- Ketik Nama Supplier --", allowClear: true, width: '100%' }); 
    });
