...
<div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        ...
        @php
            // Logika Visual: Kalau lagi diklik, tombolnya nyala terang (Ring 4)
        ...
        <a href="{{ route('dashboard', ['status' => (isset($statusFilter) && $statusFilter === 'red' ? 'all' : 'red')]) }}" class="bg-white dark:bg-[#1e293b] rounded-2xl p-6 shadow-sm border border-slate-200 dark:border-slate-800 border-l-4 border-l-red-500 flex items-center justify-between transition-all duration-300 hover:-translate-y-1 hover:shadow-lg {{ $aktifRed }}">
            <div>
                <p class="text-[10px] font-extrabold text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-1">Darurat (Merah)</p>
                <h3 class="text-3xl font-black text-slate-800 dark:text-white">
                    {{ $totalRed }} <span class="text-xs font-semibold text-slate-500 uppercase">Item</span>
                </h3>
                <p class="text-xs text-red-500 font-bold mt-2"><i class="fas fa-arrow-up mr-1"></i> Perlu Naikkan Harga</p>
            </div>
            <div class="w-14 h-14 bg-red-50 dark:bg-red-900/30 rounded-full flex items-center justify-center text-red-500 text-2xl shadow-inner">
        ...
        <a href="{{ route('dashboard', ['status' => (isset($statusFilter) && $statusFilter === 'yellow' ? 'all' : 'yellow')]) }}" class="bg-white dark:bg-[#1e293b] rounded-2xl p-6 shadow-sm border border-slate-200 dark:border-slate-800 border-l-4 border-l-amber-500 flex items-center justify-between transition-all duration-300 hover:-translate-y-1 hover:shadow-lg {{ $aktifYellow }}">
            <div>
                <p class="text-[10px] font-extrabold text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-1">Waspada (Kuning)</p>
                <h3 class="text-3xl font-black text-slate-800 dark:text-white">
                    {{ $totalYellow }} <span class="text-xs font-semibold text-slate-500 uppercase">Item</span>
                </h3>
                <p class="text-xs text-amber-500 font-bold mt-2"><i class="fas fa-balance-scale mr-1"></i> Perlu Penyesuaian</p>
            </div>
            <div class="w-14 h-14 bg-amber-50 dark:bg-amber-900/30 rounded-full flex items-center justify-center text-amber-500 text-2xl shadow-inner">
        ...
        <a href="{{ route('dashboard', ['status' => (isset($statusFilter) && $statusFilter === 'green' ? 'all' : 'green')]) }}" class="bg-white dark:bg-[#1e293b] rounded-2xl p-6 shadow-sm border border-slate-200 dark:border-slate-800 border-l-4 border-l-emerald-500 flex items-center justify-between transition-all duration-300 hover:-translate-y-1 hover:shadow-lg {{ $aktifGreen }}">
            <div>
                <p class="text-[10px] font-extrabold text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-1">Aman (Hijau)</p>
                <h3 class="text-3xl font-black text-slate-800 dark:text-white">
                    {{ $totalGreen }} <span class="text-xs font-semibold text-slate-500 uppercase">Item</span>
                </h3>
                <p class="text-xs text-emerald-500 font-bold mt-2"><i class="fas fa-check-double mr-1"></i> Harga Stabil Target</p>
            </div>
            <div class="w-14 h-14 bg-emerald-50 dark:bg-emerald-900/30 rounded-full flex items-center justify-center text-emerald-500 text-2xl shadow-inner">
    ...
    <div class="bg-white dark:bg-[#1e293b] rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 overflow-hidden transition-colors duration-300 flex flex-col">
        ...
        <div class="bg-slate-50 dark:bg-slate-900/50 px-6 py-4 border-b border-slate-200 dark:border-slate-700 flex flex-col lg:flex-row justify-between items-center gap-4">
            <h6 class="m-0 font-bold text-slate-700 dark:text-slate-200 flex items-center whitespace-nowrap">
        ...
        <div class="overflow-x-auto custom-scrollbar flex-1">
            <table class="w-full text-left border-collapse whitespace-nowrap">
                <thead class="bg-white dark:bg-[#1e293b] text-slate-500 dark:text-slate-400 text-[10px] uppercase font-extrabold tracking-wider border-b border-slate-200 dark:border-slate-700">
                    <tr>
                    ...
                    @forelse($dailyPricings as $dp)
                        ...
                        <tr class="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors group searchable-row" 
                            ...
                            <td class="px-6 py-4 text-center text-slate-600 dark:text-slate-400">
                                <div class="font-bold text-slate-800 dark:text-slate-300">{{ \Carbon\Carbon::parse($dp->date_input)->format('d M') }}</div>
                                <div class="text-[10px] font-semibold">{{ \Carbon\Carbon::parse($dp->date_input)->format('Y') }}</div>
                            </td>
                            ...
                            <td class="px-6 py-4">
                                @if($dp->product && $dp->product->category)
                                    <span class="inline-block px-2.5 py-1 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 text-[10px] font-bold uppercase tracking-wider rounded-md border border-slate-200 dark:border-slate-700">
                                        {{ $dp->product->category }}
                                    </span>
                                @else
                            ...
                            <td class="px-6 py-4 text-center">
                                @if($dp->demand === 'tinggi')
                                    <span class="inline-flex items-center px-2.5 py-1 rounded-md text-[10px] font-black bg-orange-100 text-orange-700 border border-orange-200">
                                        <i class="fas fa-fire mr-1"></i> TINGGI
                                    </span>
                                @elseif($dp->demand === 'rendah')
                                    <span class="inline-flex items-center px-2.5 py-1 rounded-md text-[10px] font-black bg-blue-100 text-blue-700 border border-blue-200">
                                        <i class="fas fa-snowflake mr-1"></i> RENDAH
                                    </span>
                                @else
                                    <span class="inline-flex items-center px-2.5 py-1 rounded-md text-[10px] font-black bg-slate-100 text-slate-600 border border-slate-200">
                                        NORMAL
                                    </span>
                                @endif
                            </td>
                            ...
                            <td class="px-6 py-4 text-right font-black text-indigo-600 dark:text-indigo-400 text-base bg-indigo-50/30 dark:bg-indigo-900/10 transition-colors">
                                {{ number_format($dp->final_price, 0, ',', '.') }}
                            </td>
                            <td class="px-6 py-4 text-center">
                                <div class="flex flex-col items-center">
                                    <span class="text-sm">{!! $trendIcon !!}</span>
                                    <span class="text-[9px] font-black {{ $trendClass }} uppercase tracking-widest mt-0.5">{{ $trendText }}</span>
                                </div>
                            </td>
                            
                            <td class="px-6 py-4 text-center font-black text-slate-700 dark:text-slate-200 text-base">
                                {{ number_format((float)$dp->margin_percent > 1 ? $dp->margin_percent : $dp->margin_percent * 100, 1) }}%
                            </td>
                            
                            <td class="px-6 py-4 text-center">
                                @if($statusFilter === 'red')
                                    <span class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-[10px] font-bold bg-red-50 text-red-700 border border-red-200 shadow-sm">
                                        <span class="w-2 h-2 rounded-full bg-red-500 animate-pulse"></span> Naikkan Harga
                                    </span>
                                @elseif($statusFilter === 'yellow')
                                    <span class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-[10px] font-bold bg-amber-50 text-amber-700 border border-amber-200 shadow-sm">
                                        <span class="w-2 h-2 rounded-full bg-amber-500"></span> Sesuaikan
                                    </span>
                                @else
                                    <span class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-[10px] font-bold bg-emerald-50 text-emerald-700 border border-emerald-200 shadow-sm">
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Mesin Filter JS (Sekarang khusus buat teks dan kategori aja!)
        function filterTable() {
            ...
            // Tampilkan radar kosong kalau nggak ada data yang cocok
            if (rows.length > 0 && jsEmptyState) {
                jsEmptyState.style.display = visibleCount === 0 ? '' : 'none';
            }
        }

        // Pasang sensor event listener
        if(searchInput) {
