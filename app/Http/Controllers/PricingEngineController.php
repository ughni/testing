   ...
   public function calculate(Request $request)
    {
        ...
        $request->validate([
            ...
            // Tambahan Validasi Bos Lu
            'buffer_setup' => 'nullable|numeric|min:0',
            'threshold_setup' => 'nullable|numeric|min:0',
            'adj_hpp_setup' => 'nullable|numeric',
            'adj_demand_setup' => 'nullable|numeric',
            'adj_stok_setup' => 'nullable|numeric',
        ], [
        ...
        // ==========================================
        // 🔥 LANGSUNG EKSEKUSI MESIN UTAMA (TANPA BYPASS) 🔥
        // Mesin akan otomatis membaca 'manual_margin' dari form.
        // ==========================================
        ...
        // ALARM MARGIN RUGI (Sudah disesuaikan pakai 'RED')
        if ($result['status_margin'] === 'RED') {
    ...
    public function updateHistory(Request $request, $id)
    {
        ...
        // ==========================================
        // 🔥 LANGSUNG EKSEKUSI MESIN UTAMA (TANPA BYPASS) 🔥
        // Biar pas ngedit data lama, rumusnya tetap sinkron!
        // ==========================================
        $result = $this->runPricingEngine($request, $product);
        ...
        $history->update([
            ...
            'c2' => $request->c2,
            'c3' => $request->c3,
            'yield_applied' => $result['yield_percent'], // Tambahin ini biar yield update juga
            'stock' => $request->stock,
            'demand' => $request->demand,
            'final_price' => $result['final_price'],
            'margin_percent' => $result['margin_percent'],
    ...
    public function runPricingEngine(Request $request, $product)
    {
        ...
        // 🔥 PERBAIKAN FATAL: DEFAULT YIELD DIJADIKAN 95% (0.95) BIAR KEMBAR SAMA LAYAR JS! 🔥
        $yield_percent = 0.95; 
        ...
        } elseif ($globalSetting && isset($globalSetting->yield_percent) && $globalSetting->yield_percent > 0) {
        ...
        // 2. AMBIL VARIABEL DARI FORM
        $buffer_percent = $request->filled('buffer_setup') ? ($request->buffer_setup / 100) : (isset($product->buffer) ? ($product->buffer / 100) : 0.05);
        if ($buffer_percent > 1) $buffer_percent = $buffer_percent / 100;

        $markup_base = $request->filled('manual_margin') ? ($request->manual_margin / 100) : (isset($product->markup) ? ($product->markup / 100) : 0.20);
        if ($markup_base >= 1) $markup_base = 0.99;

        $threshold_stock = $request->filled('threshold_setup') ? $request->threshold_setup : (isset($product->threshold) ? $product->threshold : 20);
        ...
        $adj_hpp = 0;
        $aksi_hpp_naik = $request->filled('adj_hpp_setup') ? ($request->adj_hpp_setup / 100) : ($autoAdjust ? $autoAdjust->hpp_adjustment : 0.03);

        // BANDINGKAN HPP MENTAH (DARI REQUEST)
        if ($request->hpp_prev && $request->hpp_prev > 0 && $request->hpp > $request->hpp_prev) {
            $adj_hpp = $aksi_hpp_naik;
        }

        $adj_demand = 0;
        $rule_demand_tinggi = $request->filled('adj_demand_setup') ? ($request->adj_demand_setup / 100) : ($autoAdjust ? $autoAdjust->demand_high_adjustment : 0.03);
        $rule_demand_rendah = $request->filled('adj_demand_setup') ? -abs($request->adj_demand_setup / 100) : ($autoAdjust ? -abs($autoAdjust->demand_low_adjustment) : -0.03);
        ...
        $adj_stock = 0;
        $rule_stok_menipis = $request->filled('adj_stok_setup') ? ($request->adj_stok_setup / 100) : ($autoAdjust ? $autoAdjust->stock_low_adjustment : 0.02);
        ...
        // 4. LOGIKA KOMPETITOR
        $competitorPrices = array_filter([$request->c1, $request->c2, $request->c3], function($val) {
        ...
        if (count($competitorPrices) > 0) {
            ...
            $temp_price = $adjusted_price; // Monopoli!
        }
        ...
        // 6. KALKULASI MARGIN AKTUAL & STATUS
        ...
        if ($marginPercent < $buffer_percent) {
            ...
            $statusMargin = 'YELLOW'; 
            $recMsg = '🟡 Waspada (YELLOW). Margin aktual di bawah target ideal. Cek demand atau kompetitor.';
        } else {
            $statusMargin = 'GREEN'; 
            $recMsg = '🟢 Aman (GREEN). Kalkulasi Selesai. Harga rekomendasi sangat optimal.';
        }
